...
public class SVRLValidationFilterTest {
    ...
    public void testIncorrectValidationReport() throws IOException, SAXException {
        InputStream inputStream = getClass().getClassLoader().getResourceAsStream("se/pagero/schematron/filter/svrl-validationreport-error.xml");
        ...
        Assert.assertEquals(validationFilter.getErrorReport(), "[BIICORE-T10-R000]-This XML instance is NOT a core BiiTrdm010 transaction Failed when performing test [contains(cbc:CustomizationID, 'urn:www.cenbii.eu:transaction:biicoretrdm010:ver1.0')].\n" +
    ...
    public void testCorrectValidationReport() throws IOException, SAXException {
        InputStream inputStream = getClass().getClassLoader().getResourceAsStream("se/pagero/schematron/filter/svrl-validationreport-ok.xml");
        SVRLValidationFilter validationFilter = createAndInitiateValidationFilter();
        validationFilter.parse(new InputSource(inputStream));
